...
class GameScreen extends Component {
   ...
   setMatrix = (matrix, player) => {
       
        
        const matrixToBeSet = matrix == null ? [...Array(this.gridSize)].map(e => Array(this.gridSize).fill(0)) : matrix
        const history = [...this.state.history, matrix]
      
        this.setState({matrix: matrixToBeSet, player: player, history:history}, () =>  console.log("history",this.state.history, matrixToBeSet));
                  ...
                  <SafeAreaView>   
                        <View style={styles.outerContainer}>
                            <View style={this.containerStyle}>
                                <CrossLayout size={3} width={this.windowWidth}/>      
                            </View>
                            <View style={this.containerStyle}>
